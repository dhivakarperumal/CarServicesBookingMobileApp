...
export default function CarsScreen() {
  ...
  const updateStatus = async (item, newStatus) => {
    ...
    if (
      ...
      Alert.alert("Add Parts or Issues First");
      return;
    }
  ...
  /* 🔧 PARTS MODAL FUNCTIONS (UNCHANGED) */
  const openPartsModal = (item) => {
    setSelectedCar(item);
    setParts([{ partName: "", qty: 1, price: 0 }]);
    setPartsModal(true);
  ...
  const saveIssues = async () => {
    ...
    const validIssues = issues.filter((i) => i.issue);

    if (validIssues.length === 0) {
      Alert.alert("Add at least one issue");
      return;
    }
    ...
    try {
      ...
      console.log(err);
      Alert.alert("Failed to save issues");
    } finally {
